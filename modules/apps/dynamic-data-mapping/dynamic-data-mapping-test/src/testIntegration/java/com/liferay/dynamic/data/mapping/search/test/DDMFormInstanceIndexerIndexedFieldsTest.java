...
public class DDMFormInstanceIndexerIndexedFieldsTest {
	...
	public void testIndexedFields() throws Exception {
		...
		indexerFixture.reindex(ddmFormInstance.getCompanyId());
		...
		FieldValuesAssert.assertFieldValues(
			_expectedFieldValues(ddmFormInstance), document, searchTerm);
	}
	...
	protected void setUpDDMFormInstanceIndexerFixture() {
		indexerFixture = new IndexerFixture<>(DDMFormInstance.class);
	}
	...
	protected IndexerFixture<DDMFormInstance> indexerFixture;
	...
	private Map<String, String> _expectedFieldValues(DDMFormInstance ddmForm)
		throws Exception {

		Map<String, String> map = new HashMap<>();

		map.put(
			Field.ENTRY_CLASS_PK, String.valueOf(ddmForm.getFormInstanceId()));
		map.put(Field.ENTRY_CLASS_NAME, DDMFormInstance.class.getName());
		map.put(Field.COMPANY_ID, String.valueOf(ddmForm.getCompanyId()));
		map.put(Field.GROUP_ID, String.valueOf(ddmForm.getGroupId()));
		map.put(Field.USER_ID, String.valueOf(ddmForm.getUserId()));
		map.put(Field.USER_NAME, StringUtil.lowerCase(ddmForm.getUserName()));
		map.put(Field.SCOPE_GROUP_ID, String.valueOf(ddmForm.getGroupId()));
		map.put(Field.STAGING_GROUP, String.valueOf(group.isStagingGroup()));

		indexedFieldsFixture.populateUID(
			DDMFormInstance.class.getName(), ddmForm.getFormInstanceId(), map);

		_populateDates(ddmForm, map);

		_populateRoles(ddmForm, map);
